...
UPTIME = tcr.Uptime().get().delta.microseconds
VERSION = '2.0.%s' % p.Path('VERSION.txt').read_text().strip()
...
REVOLUTION = -1  # -1 Means not fetched yet
DB_USERS_PATH = S.DB_DIRECTORY / 'users'
DB_USERS_PATH.mkdir(exist_ok=True)  # fmt: skip
DB_VERSION_PATH = S.DB_DIRECTORY / 'version'
DB_VERSION_PATH.mkdir(exist_ok=True)  # fmt: skip
DB_GLOBAL_PATH = S.DB_DIRECTORY / 'global'
DB_GLOBAL_PATH.mkdir(exist_ok=True)  # fmt: skip
