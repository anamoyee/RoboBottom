...
if True:  # \/ Bot-dependant funcs
	...
	async def reminder_scheduler(content: str, responder, author_id, *, force_ephemeral=False, reply_to=hikari.UNDEFINED):
		...
		try:
			...
			if ((not testmode()) and (unix < S.LIMITS.MINIMAL_DURATION)) or unix < 2:
				await responder(
					f":x: That's too soon! Make sure the delay is at least `{S.LIMITS.MINIMAL_DURATION if not testmode() else 2}` seconds long!" + (f" (`{unix}`)" if testmode() else ""),
					flags=message_flags,
					reply=reply_to,
				)
				return
			...
			if schedule_reminder(author_id, text=text, unix=math.floor(time.time() + unix), flags=flags):
				await responder(
					f"🔔 **{random_sure()}** I'll remind you in **{seconds_to_timestr(unix)}**" + (" ;)" if flags & ReminderFlag.HIDDEN else ""),
					flags=message_flags,
					reply=reply_to,
				)
			else:
	...
	async def send_reminder_view(responder: Callable, reminder: Reminder, rem_id):
		...
		message = await responder(EMBEDS.reminder(reminder, rem_id))  # , components=view.build())
...
"""[1:-1],
	...
	async def cmd_delhistory(ctx: lb.SlashContext):
		if ctx.guild_id is not None:
			return await ctx.respond("You may only use this command in DMs", flags=hikari.MessageFlag.EPHEMERAL)
		if ctx.options.doasisay != "Yes, do as I say!":
			return await ctx.respond("You did not type the phrase correctly, make sure to use this command carefully!")
...
"""[1:-1],
	...
	@bot.command
	@lb.option("coroutines", "If result is a coroutine, await it? (True by default)", type=bool, required=False)
	@lb.option("showcode", "Show the code AND result? (ephemeral by default)", type=bool, required=False)
	...
	async def mazerunner(ctx, code, ephemeral, *, showcode=False, await_coroutines=True):  # Maze runner? I meant devrunner... ;3
		async def r(*args, force_ephemeral=False, **kwargs):
			if ephemeral or force_ephemeral:
				kwargs["flags"] = hikari.MessageFlag.EPHEMERAL
			await ctx.respond(*args, **kwargs)
	...
	async def cmd_remind(ctx: lb.SlashContext):
		...
		if ctx.author.id == S.DEV_ID and content.startswith("$"):
			...
			if content.startswith("$"):
				...
				await mazerunner(ctx, content, ephemeral and ephoverride, showcode=showcode, await_coroutines=coroutines)
				return
			...
			match method:
				case "^":  # mkdir path
					...
					try:
						...
						udb["pdb"] = ter.traverse(udb["pdb"], "/".join(strpaths[:-1]), modify_last=ter.t.dir(strpaths[-1]), append=True)
					else:
